...
Register-ArgumentCompleter -Native -CommandName 'uvx' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'uvx' {
            ...
            [CompletionResult]::new('--native-tls', '--native-tls', [CompletionResultType]::ParameterName, 'Whether to load TLS certificates from the platform''s native store [env: UV_NATIVE_TLS=]')
