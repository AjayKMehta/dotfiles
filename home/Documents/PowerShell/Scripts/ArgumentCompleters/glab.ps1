...
[scriptblock]${__glabCompleterBlock} = {
    param(
            $WordToComplete,
            $CommandAst,
            $CursorPosition
        ...
        switch ($Mode) {
            ...
            "Complete" {
                ...
                if ($Values.Length -eq 1) {
                    ...
                    $CompletionText = $($comp.Name | __glab_escapeStringWithSpecialChars) + $Space
            ...
            Default {
                ...
                $CompletionText = $($comp.Name | __glab_escapeStringWithSpecialChars)
                if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage"){
                    [System.Management.Automation.CompletionResult]::new($CompletionText, "$($comp.Name)", 'ParameterValue', "$($comp.Description)")
                } else {
                    $CompletionText
...
Register-ArgumentCompleter -CommandName 'glab' -ScriptBlock ${__glabCompleterBlock}
