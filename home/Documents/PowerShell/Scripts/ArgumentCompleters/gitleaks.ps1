# powershell completion for gitleaks                             -*- shell-script -*-
...
[scriptblock]${__gitleaksCompleterBlock} = {
    param(
            $WordToComplete,
            $CommandAst,
            $CursorPosition
        ...
        switch ($Mode) {
            ...
            "Complete" {
                ...
                if ($Values.Length -eq 1) {
                    ...
                    $CompletionText = $($comp.Name | __gitleaks_escapeStringWithSpecialChars) + $Space
            ...
            Default {
                ...
                $CompletionText = $($comp.Name | __gitleaks_escapeStringWithSpecialChars)
                if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage"){
                    [System.Management.Automation.CompletionResult]::new($CompletionText, "$($comp.Name)", 'ParameterValue', "$($comp.Description)")
                } else {
                    $CompletionText
...
Register-ArgumentCompleter -CommandName 'gitleaks' -ScriptBlock ${__gitleaksCompleterBlock}
